...
### Que tal incluir uma FAKE API no projeto?

<br>

Dica: muitos **processos seletivos** de *pessoa desenvolvedora frontend* pedem para buscarmos dados em uma API, então criar uma fake API nos acostuma a usar esse tipo de recurso desde cedo e simular situações mais próximas do que aprenderemos no dia a dia.

<br> 

Usaremos a biblioteca [Mirage](https://miragejs.com/quickstarts/react/develop-a-component/) para a criação da fake API e a biblioteca [Axios](https://github.com/axios/axios) para fazer as requisições HTTP.
...
- Em `src` cria uma pasta `service`, onde colocaremos a fake api (que no futuro pode ser substituída por uma API de fato) e o nosso service de pessoas.

<br>

- Em `src/service` crie o arquivo `fake-api.js`
...
export function makeServer({ environment = "development" } = {}) {
  const server = createServer({
    ...
    seeds(server) {
      server.db.loadData({
        // se quiser começar com dados mocados, insira no array abaixo
        pessoas: [],
      });
    },
    ...
    routes() {
      ...
      // CREATE
      this.post("/pessoas", (schema, request) => {
      ...
      // EDIT
      this.patch("/pessoas/:id", (schema, request) => {
      ...
      // DELETE
      this.delete("/pessoas/:id", (schema, request) => {
...
// // Se você não deseja que sua fake api rode em produção descomente o if abaixo
...
- Em `src/service` crie o arquivo `index.js`
...
class PessoaService {
  // CREATE
  async adicionaPessoa(pessoa) {
    try {
      const response = await axios.post(`/api/pessoas`, pessoa);
  ...
  // EDIT
  async editaPessoa(id, pessoa) {
    try {
      const response = await axios.patch(`/api/pessoas/${id}`, pessoa);
...
- Em `src/pages/Home/index` importe o Service e busque as pessoas cadastradas.
...
function Home() {
  ...
  useEffect(() => {
    PessoaService.buscaPessoas().then((response) => setPessoas(response));
  }, []);
